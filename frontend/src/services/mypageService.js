...
function formatExpiryLabel(value, status) {
  if (status === "USED") return "?¬ìš© ?„ë£Œ";
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return "";
  return `${String(date.getMonth() + 1).padStart(2, "0")}.${String(date.getDate()).padStart(2, "0")} ë§Œë£Œ`;
}
...
function mapUserCouponDto(dto) {
  ...
  return {
    ...
    discountLabel: coupon.discountType === "RATE" ? `${coupon.discountValue}%` : `${Number(coupon.discountValue ?? 0).toLocaleString()}¿ø`,
    status: couponStatus,
...
function mapCouponCatalogDto(dto) {
  ...
  return {
    ...
    discountLabel: dto.discountType === "RATE" ? `${dto.discountValue}%` : `${Number(dto.discountValue ?? 0).toLocaleString()}¿ø`, 
