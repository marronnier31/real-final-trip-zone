  ...
  return (
    <div className="booking-form-surface">
      <div className={`booking-login-banner${authSession ? " is-active" : ""}`}>
        <div>
          <strong>
            {authSession ? `${authSession.name}님 예약을 진행합니다` : "로그인 후 예약이 가능합니다."}
          </strong>
          <p>
            {authSession
              ? "예약 후에도 체크인 전까지 일정과 요청사항을 마이페이지에서 바로 수정할 수 있어요."
              : "예약 내역과 혜택, 마일리지 사용을 위해 먼저 로그인해 주세요."}
          </p>
        </div>
        {!authSession && (
      ...
      <div className="booking-form-mock booking-form-grid">
        ...
        <label className="booking-field booking-field-full">
          ...
          <textarea
            ...
            rows="3"
            value={form.request}
            placeholder="미리 체크인, 침대, 요청 사항이 있으면 적어 주세요."
  ...
  return (
    <div className="booking-payment-card">
      <div className="booking-payment-head">
        ...
        <p>
          {selectedPayment.label} · {selectedPayment.pg}
        </p>
      </div>
      ...
      <div className="booking-summary-box">
        ...
        <div className="booking-summary-row">
          <span>쿠폰 할인</span>
          <strong>{couponDiscount > 0 ? `-${couponDiscount.toLocaleString()}원` : "할인 없음"}</strong>
        </div>
        <div className="booking-summary-row">
          <span>마일리지 사용</span>
      ...
      <button
        ...
        onClick={onSubmit}
      >
        {isSubmitting ? "예약을 처리하는 중..." : authSession ? "결제하고 예약 완료" : "로그인 후 예약 진행"}
      </button>
    </div>
  );
}
